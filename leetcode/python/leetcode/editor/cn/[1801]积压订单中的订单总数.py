...
class Solution:
    def getNumberOfBacklogOrders(self, orders: List[List[int]]) -> int:
        ...
        for order in orders:
            if order[2] == 0:
                # buy
                if len(sell_q) == 0 or sell_q[0][0] > order[0]:
                    heapq.heappush(buy_q, [-order[0], order[1]])
                    continue
                amount = order[1]
                while amount and len(sell_q) != 0 and sell_q[0][0] <= order[0]:
                    top = heapq.heappop(sell_q)
                    if amount >= top[1]:
                        amount -= top[1]
                    else:
                        top[1] -= amount
                        amount = 0
                        heapq.heappush(sell_q, top)
                        break
                if amount != 0:
                    heapq.heappush(buy_q, [-order[0], amount])
            else:
                # sell
                if len(buy_q) == 0 or -buy_q[0][0] < order[0]:
                    heapq.heappush(sell_q, [order[0], order[1]])
                    continue
                amount = order[1]
                while amount and len(buy_q) != 0 and -buy_q[0][0] >= order[0]:
                    top = heapq.heappop(buy_q)
                    if amount >= top[1]:
                        amount -= top[1]
                    else:
                        top[1] -= amount
                        amount = 0
                        heapq.heappush(buy_q, top)
                        break
                if amount != 0:
                    heapq.heappush(sell_q, [order[0], amount])
        result = 0
        for item in sell_q:
            result += item[1]
        for item in buy_q:

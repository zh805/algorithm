...
class Bank:
    ...
    def transfer(self, account1: int, account2: int, money: int) -> bool:
        # 转账
        if not self.validate(account=account1) or not self.validate(account=account2) or not self.validate(money=money):
            return False
    ...
    def deposit(self, account: int, money: int) -> bool:
        # 存款
        if not self.validate(account=account, money=money):
            return False
    ...
    def validate(self, account: int=None, money: int = None) -> bool:
        if account:

...
class Solution:
    def minNumberOfHours(self, initialEnergy: int, initialExperience: int, energy: List[int],
        ...
        方法1：遍历模拟
        """
        train_energy, train_experience = 0, 0
        cur_ex = initialExperience
        s = sum(energy)
        if initialEnergy > s:
            train_energy = 0
        else:
            train_energy = s + 1 - initialEnergy

        for ex in experience:
            if cur_ex <= ex:
                train_experience += 1 + (ex - cur_ex)
                cur_ex = ex + 1
            cur_ex += ex

        return train_experience + train_energy
...
if __name__ == '__main__':
    ...
    energy = [1, 4, 3, 2]
    experience = [2, 6, 3, 1]
    result = Solution().minNumberOfHours(initialEnergy, initialExperience, energy, experience)
    print(result)
